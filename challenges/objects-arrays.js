...
function namesAllLower(zooAnimals) {
  return lowCaseAnimalNames.map(function(value1) {
    return value1.animal_name.toLowerCase();
  });
}
...
function checkPop(population) {
  return zooAnimals.population <= 4;
}
function newFunc() {
  zooAnimals.population.filter(checkPop)
}
lowPopulationAnimals.push(checkPop);
...
function totalPop(currentPop, callback) {
  const sum = currentPop.reduce((accumulator, currentValue) => accumulator + currentValue, 0)
